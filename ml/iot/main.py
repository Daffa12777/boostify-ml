...
# CONFIG
# =========================================================
API_URL ="https://web-boostify.vercel.app/api/uploadfromml"
IDLE_TIMEOUT = 10
...
# =========================================================
# PESAN MOTIVASI (tanpa emoji agar bisa dibaca espeak)
# =========================================================
PESAN = {
    "Pagi": [
        "Semangat kuliahnya!",
...
def tampilkan_standby():
    ...
    text = font_pesan.render("Arahkan wajah ke kamera...", True, (180, 180, 255))
    screen.blit(text, text.get_rect(center=(240, 200)))
    pygame.display.flip()

def tampilkan_greeting(name, confidence, is_smiling, motivasi):
    waktu = get_waktu()
    nama_display = name.capitalize()
    screen.fill((20, 20, 40))
    pygame.draw.circle(screen, (50, 50, 100), (240, 80), 60)
    t1 = font_pesan.render(f"Selamat {waktu},", True, (180, 180, 255))
    screen.blit(t1, t1.get_rect(center=(240, 150)))
    t2 = font_nama.render(nama_display + "!", True, (255, 220, 50))
    screen.blit(t2, t2.get_rect(center=(240, 195)))
    pesan = "Senyumnya Keren!" if is_smiling else motivasi
    t3 = font_pesan.render(pesan, True, (150, 255, 150))
    screen.blit(t3, t3.get_rect(center=(240, 245)))
    conf_text = f"Confidence: {confidence*100:.1f}%"
    t4 = font_kecil.render(conf_text, True, (120, 120, 120))
    screen.blit(t4, t4.get_rect(center=(240, 295)))
    pygame.display.flip()

def tampilkan_unknown():
    screen.fill((70, 0, 0))
    text1 = font_nama.render("Tidak Dikenal", True, (255, 255, 255))
    screen.blit(text1, text1.get_rect(center=(240, 140)))
    text2 = font_pesan.render("Coba Lagi", True, (255, 180, 180))
...
def kirim_absen(result):
    ...
    if current_time - last_upload_time[nama] <= COOLDOWN_SEC:
        return
    try:
        response = requests.post(API_URL, json=result, timeout=5)
...
def main():
    ...
    while True:
        ...
        if status == "recognized":
            ...
            motivasi   = random.choice(PESAN[get_waktu()])

            tampilkan_greeting(nama, confidence, is_smiling, motivasi)

            if is_smiling:
                pesan_audio = f"Selamat datang {nama}, senyumnya keren. {motivasi}"
            else:
                pesan_audio = f"Selamat datang {nama}. {motivasi}"
